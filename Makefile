...
# --- Timer1, de 16 bits ---
# El que trae la trampa numero 4: el registro TEMP, que es UNO SOLO y
# COMPARTIDO entre TCNT1, ICR1, OCR1A y OCR1B. simavr no lo modela.
.PHONY: sim-timer1
sim-timer1:
	@verilator --cc --exe --build -Wall -Wno-DECLFILENAME \
	  $(INCDIRS) -Mdir $(BUILD)/vtimer1 -o tb_timer1 \
	  --top-module tb_timer1_top sim/periph/tb_timer1_top.v \
	  rtl/periph/axioma_timer1.v rtl/periph/axioma_prescaler.v \
	  sim/periph/tb_timer1.cpp >/dev/null
	@echo -e "$(BOLD)Timer1 de 16 bits contra la hoja de datos$(NC)"
	@./$(BUILD)/vtimer1/tb_timer1
...
sim-pud:
	@verilator --cc --exe --build -Wall -Wno-DECLFILENAME \
	  $(INCDIRS) -Mdir $(BUILD)/vpud -o tb_soc_pud --top-module tb_soc_clk_top \
	  sim/soc/tb_soc_clk_top.v $(SOC_SRCS) sim/soc/tb_soc_pud.cpp >/dev/null
	@echo -e "$(BOLD)PUD: el apagado global de los pull-up$(NC)"
	@./$(BUILD)/vpud/tb_soc_pud

# --- el sueno, con la cascara del banco del reloj ---
.PHONY: sim-sleep
sim-sleep:
	@verilator --cc --exe --build -Wall -Wno-DECLFILENAME \
	  $(INCDIRS) -Mdir $(BUILD)/vslp -o tb_soc_sleep --top-module tb_soc_clk_top \
	  sim/soc/tb_soc_clk_top.v $(SOC_SRCS) sim/soc/tb_soc_sleep.cpp >/dev/null
	@echo -e "$(BOLD)El sueno: los dos relojes son dos$(NC)"
	@./$(BUILD)/vslp/tb_soc_sleep
...
# ------------------------------------------------- la regresion entera
# LA LISTA DE LO QUE TIENE QUE PASAR VIVE AQUI Y EN NINGUN OTRO SITIO. Estaba
# copiada en el README, en INSTALL.md y en el flujo de la CI, y tres copias de
# una lista son tres cifras que se desincronizan: ya paso con la cuenta de
# objetivos. Ahora los tres citan este objetivo.
#
...
check-all:
	...
	for t in $(REGRESION); do \
	  if $(MAKE) --no-print-directory $$t >$(BUILD)/check-$$t.log 2>&1; then \
	    ...
	    echo -e "  $(RED)FALLA$(NC) $$t   $(DIM)($(BUILD)/check-$$t.log)$(NC)"; \
	    ko=$$((ko+1)); malos="$$malos $$t"; \
	  fi; \
	done; \
	echo ""; \
	if [ $$ko -eq 0 ]; then \
	  echo -e "  $(GREEN)$$ok/$$ok objetivos en verde$(NC)"; \
	else \
	  echo -e "  $(RED)$$ok en verde, $$ko en rojo:$$malos$(NC)"; \
	fi; \
	test $$ko -eq 0
