...
├── axioma_usart           asíncrono · síncrono por XCK (PD4) · MPCM · SPI maestro
...
| El modo síncrono, `MPCM` y el **SPI maestro** | **simavr no los modela en absoluto.** No tiene pin `XCK` ni nada que se le parezca, y `MPCM0` es para él un bit de almacenamiento. Si no serializa el modo asíncrono, menos aún uno donde el reloj es una señal del encapsulado. | **El mismo banco propio**, con un extremo síncrono colgado de `XCK`: el periodo se mide contra `f_XCK = f_CPU/(2·(UBRR+1))`, y `UCPOL` se comprueba **en el flanco** —que `TXD` esté quieto alrededor del de muestreo—, no por que el dato llegue. Con los dos extremos equivocados de la misma manera el dato llega perfecto, que es el fallo que este proyecto ya conoce. Para `MSPIM`, un **esclavo SPI** al otro lado del cable: los cuatro modos por los dos órdenes de bit, y que una trama sean **ocho pulsos y ni uno más**. |
